...
my %cmdLOptions = ( 
                    ...
                    "ribsize|t=s"           => \$options{'ribsize'},
                    "ribfileslist|rl=s"     => \$options{'ribfileslist'},    # [Rachita/Jie]
...
## if user didn't supply ribfileslist
if (!defined($options{'ribfileslist'})) {
  &debug($progname, "[no ribfileslist provided, using ribsize or ribfile]") if $DEBUG_VERBOSE;

  ## get RIB table size if user supplied a ribfile (instead of table size)
  if (!defined($options{'ribsize'})) {
    $options{'ribsize'} = &getTableSize($options{'ribfile'}, $options{'peer'}, \%fieldIndex);
    if ($options{'ribsize'} == -1) {
      &logError($progname, "Errors occured while processing ribfile");
      exit($error);
    }
  }

  ## either we were unable to find table size of user input wrong table size
  if ($options{'ribsize'} <= 0) {
    &logError($progname, "Cannot proceed with a table (RIB) size of: ".  "$options{'ribsize'}");
    exit($error);
  }

  ## Expected Table Size (fraction of all table prefixes)
  $expectedTableSize = $options{'ribsize'} * $options{'percent'};
}
## user provided ribfileslist
else {
  &debug($progname, "[using ribfileslist]") if $DEBUG_VERBOSE;
  if (! -T $options{'ribfileslist'}) {
    &logError($progname, "Invalid ribfileslist file "."$options{'ribfileslist'}: not a plain text file");
    exit($error);
  }
  ## check if it is empty
  if (-z $options{'ribfileslist'}) {
    &logError($progname, "Invalid ribfileslist file "."$options{'ribfileslist'}: file is empty");
    exit($error);
  }
}
...
if ($DEBUG_DETECTD) {
  &debug($progname, "All collection time:");
  foreach my $ct (@ctimes) {
    &debug($progname, "$$ct[0]   $$ct[1]   $$ct[2]");
  }
}
...
if (&getLocalMinimaList(\@ctimes, \@localMinima)) {
  &logError($progname, "Errors occured while getting local minima list");
  exit($error);
}
#use Data::Dumper;
#print scalar(@localMinima), "\n";
#print Dumper(\@localMinima);
...
if ($DEBUG_DETECTD) {
  ...
  foreach my $lcmin (@localMinima) {
    ...
    &debug($progname, sprintf("%s   %s   %s", $$lcmin[0], $$lcmin[1], $ctimes[$$lcmin[2]]->[2]));
  }
}
...
if (&bottomSearch(\@ctimes, \@detected, \@bottomSearched, $options{'b_x'}, $options{'b_s'}, $options{'b_e'}, \@endPoint)) {
...
##  arg 2: string, representing peer
...
##  return table size (from ribfile) for a given peer
  ...
  my ($ribFile, $nextHopPeer, $rfFieldIndex) = @_;
  ...
  $ribPrefixes{$prefix} = 0 if $peer eq $nextHopPeer;
  ...
  while($line = <RIBFILE>) {
    ...
    next if $peer ne $nextHopPeer;
    $ribPrefixes{$prefix} = 0;
  }

  ## table size
  my $tableSize = scalar keys %ribPrefixes;
  &debug($name, "Found table size for $peer from $ribFile: $tableSize") if $DEBUG_VERBOSE;
...
## input:
##  arg 1: string, representing ribfiles list
##  arg 2: string, representing updatefiles list
##  arg 3: reference to list, representing collection times
##                    (ref->ctimes[i] ===> ref->[ts, s(t)])
##  arg 4: string, representing peer for which collection times are calculated
##  arg 5: int, representing 'range' seconds (upper bound on s(t) values)
##  arg 6: reference to hash, representing Field:Index pairs
##  arg 7: int, representing table size estimate
##  arg 8: [optional] string, representing ctimesfile (for debugging)
## output:
...
##  matter how many updates happen within one timestamp).
  ...
  if (scalar @_ < 7 && scalar @_ > 8) {
    &logError($name, "Expecting 7 or 8 arguments, but received (@_) instead");
    return $error;
  }

  my ($ribFilesListFile, $updateFilesListFile, $rfCtimes, $nextHopPeer, $range, $rfFieldIndex, $nRIB, $ctimeFile);

  ##----------------------------------------------------------
  ## Read in parameters
  ##----------------------------------------------------------
  if (scalar @_ == 7) {
    ($ribFilesListFile, $updateFilesListFile, $rfCtimes, $nextHopPeer, $range, $rfFieldIndex, $nRIB) = @_;
    $ctimesFH = *STDERR if $DEBUG_CTIME;
  }
  else {
    ($ribFilesListFile, $updateFilesListFile, $rfCtimes, $nextHopPeer, $range, $rfFieldIndex, $nRIB, $ctimeFile) = @_;
  ...
  ##----------------------------------------------------------
  ## Prepare rib files list
  ##----------------------------------------------------------
  ## if $nRIB equals 0, it means user provides ribfileslist
  ## need processes it and store into @ribFilesList array
  my @ribFilesList; # list of rib files to be processed
  my $useribFilesList = 0;
  if ($nRIB == 0)
  {
    $useribFilesList = 1;
    if (!open(RIBFLIST, "< $ribFilesListFile")) {
      &logError($name, "Error opening ribFilesList file: ". "$options{'ribfileslist'}: $!");
      return $error;
    }

    &debug($name, "Opened ribFilesList file: $options{'ribfileslist'} for reading") if $DEBUG_VERBOSE;

    while (my $file = <RIBFLIST>) {
      chomp($file);
      $file =~ s/\s*$//; # strip whitespace at end of filename
      if (! -e $file) {
        &logError($name, "Unable to find a rib file $file ".  "(from ribFilesList list): File does not exist");
        return $error;
      }
      ...
      push(@ribFilesList, $file);
    }
    close (RIBFLIST);

    ## ribFilesList file may list rib files in any order,
    ## but we need them in cronological order
    @ribFilesList = sort(@ribFilesList);
  } 
  ...
  my $count=0;		         # number of files
   
  foreach my $updateFile (@updateFilesList) {

    ##
    ## check $useribFilesList flag to see if need process ribfile
    ##
    if ($useribFilesList) {
      my $ribFile = "";
      ## load RIB table  

      if ($count < scalar(@ribFilesList) and
          ($count == 0 or ($count > 0 and ($ribFilesList[$count] ne $ribFilesList[$count-1])))
         )
      {
        $ribFile = $ribFilesList[$count];

        $nRIB = &getTableSize($ribFile, $options{'peer'}, \%fieldIndex);	
        $nRIB = $nRIB * $options{'percent'};
        &debug($name, "Change Table Size: $nRIB") if $DEBUG_VERBOSE;    

### [DEBUG]
#my $ribFile_next = $ribFilesList[$count+1];
#my $nRIB_next    = &getTableSize($ribFile_next, $options{'peer'}, \%fieldIndex);	
#$nRIB_next = $nRIB_next * $options{'percent'};


#194.68.129.195
#print STDERR "===============================\n";
#print STDERR $nRIB, "\n";
#print STDERR $nRIB_next, "\n";
#print STDERR ($nRIB_next/$nRIB), "\n";
#
#if ($nRIB_next > $nRIB)
#{
#  $nRIB = $nRIB_next;
...
### [/DEBUG]
        ...
        if ($nRIB <= 0) {
          &logError($progname, "Invalid RIB size: $nRIB");
          return $error;
        }
      }
      $count ++;
      ## Notes: user need make sure @ribFilesList and @updateFilesList are matched
      &debug($name, "[ribFile] $ribFile")        if $DEBUG_VERBOSE;    
      &debug($name, "[updateFile] $updateFile")  if $DEBUG_VERBOSE;    
      &debug($name, "Current Table Size: $nRIB") if $DEBUG_VERBOSE;    
    }
    ...
    while($line = <UPDFILE>) {
      ...
      ## ignore 'STATE' and 'W'ithdraw messages
      next if $aws eq 'STATE' or $aws eq 'W';
      ## ignore other peers (we only work on one peer at a time)
      next if $peer ne $nextHopPeer;
      ## else if $aws eq 'A'
      ...
      if ($ts > $end)
      {
        $uniq_size = scalar keys %updatePrefixes;
        ## Accumulate updates (uniq prefixes) until:
        ##  - current updatesPrefixes > RIB estimate,
        ##     (OR)
        ##  - we have reached the maximum s(t) value, ie., "range" seconds
        ##
        ...
        if ($uniq_size >= $nRIB or ($end - $start) >= $range)
        {
          ##
          ## current updatePrefixes > RIB estimate
          ##
          while(($uniq_size >= $nRIB) && ($start < $end)) {
            ## advance "start" marker to the next timestamp in queue
            while(${$queue[0]}[0] == $start) {
              my $que_ref = shift(@queue);
              $updatePrefixes{$$que_ref[1]}--;
              delete $updatePrefixes{$$que_ref[1]} if $updatePrefixes{$$que_ref[1]} == 0;
            }

            #push(@$rfCtimes, [$start,$end - $start]); ## [DEBUG #0: Time granularity] 
            #print $ctimesFH ("$start  ", $end - $start + 1, "\n") if $DEBUG_CTIME;
            #push(@$rfCtimes, [$start,$end - $start + 1]); ## [DEBUG #0: Time granularity] 
            push(@$rfCtimes, [$start,$end - $start + 1, $uniq_size]); ## [DEBUG #0: Time granularity] 
            print $ctimesFH ("$start  ", $end - $start + 1, "\n") if $DEBUG_CTIME;

            # Reset $start and exit if @queue is empty
            if ( scalar(@queue) <=0 ) { $start = undef; last; }
            $start = ${$queue[0]}[0];
            $uniq_size = scalar keys %updatePrefixes;
          }
          ...
          while(($end - $start) >= $range) {
            ...
            push(@$rfCtimes, [$start,$range, $uniq_size]);
            print $ctimesFH ("$start  ", $range, "\n") if $DEBUG_CTIME;
      ...
      push(@queue, [$ts, $prefix]); # enqueue updates
    ...
    #}
    close(UPDFILE);
    &debug($name, "Finished processing update file: $updateFile") if $DEBUG_VERBOSE;
  } ## end foreach ..
